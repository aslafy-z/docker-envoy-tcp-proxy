...
ADMIN_ENDPOINT=${ADMIN_ENDPOINT:-0}
ADMIN_ADDRESS=${ADMIN_ADDRESS:-0.0.0.0}
ADMIN_PORT=${BIND_PORT:-9901}
...
if [ "$ADMIN_ENDPOINT" = 0 ]; then
  sed -i \
    -e "/__TMPL_BEGIN_ADMIN_ENDPOINT__/,/__TMPL_END_ADMIN_ENDPOINT__/d" \
    /usr/local/etc/envoy/config.yml
else
  sed -i \
    -e "/__TMPL_BEGIN_ADMIN_ENDPOINT__/d" \
    -e "/__TMPL_END_ADMIN_ENDPOINT__/d" \
    -e "s/__TMPL_ADMIN_ADDRESS__/$ADMIN_ADDRESS/g" \
    -e "s/__TMPL_ADMIN_PORT__/$ADMIN_PORT/g" \
    /usr/local/etc/envoy/config.yml
fi
